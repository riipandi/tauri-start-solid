...
export function ToastProvider() {
  return (
    <BaseToast.Region>
      <BaseToast.List class='fixed bottom-6 right-6 z-50 flex flex-col gap-2 w-72 max-w-[100vw] list-none p-0 m-0 outline-none' />
    </BaseToast.Region>
  )
}
...
export function Toast(props: {
  ...
  return (
    <BaseToast.Root
      ...
      class={clsx(
        'relative flex items-start gap-2.5 py-2 px-3 rounded-lg shadow-raised',
        ...
        props.type === 'success'
          ? 'bg-background-positive text-on-background-positive'
          : 'bg-background-critical text-on-background-critical'
      )}
    >
      ...
      <div class='flex-1 min-w-0'>
        <BaseToast.Title class='text-[13px] font-semibold mb-0.5'>
          {props.type === 'success' ? 'Success' : 'Error'}
        </BaseToast.Title>
        <BaseToast.Description class='text-[13px] opacity-90'>{props.title}</BaseToast.Description>
      </div>
